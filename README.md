...
Supported parameters: `recids` (dash-separated record IDs), `align=true` (align timelines), `rate=true` (citation rate view), `est` (rate estimator: `binned` or `blocks`; smooth is the default), `yearly=true` (overlay the actual per-year citation counts on the rate view), `p0` (Bayesian-blocks sensitivity: `0.3` or `0.9`; `0.05` is the strict default), `wsb=true` (overlay the Wang–Song–Barabási model fit), and `bin` (smoothing scale / bin width in months: 1, 3, 6, or 12; omit for automatic)
...
**Embedding**: `?embed=1` renders the chart alone (no controls), for iframes. After every render the page posts a `{source: 'inspire-citation-history', type: 'stats', series: [...]}` message to its parent with each record's citation total (INSPIRE's `citation_count`, re-read on every visit), the times INSPIRE was last asked for the count and for the citing-record list (`countFetchedAt`, `fetchedAt`, ms since epoch — the list may come from the day-long cache), the number of citing records dated in the trailing 90 days (`recent: {windowDays, count}`, from the same list, a record dated ahead of today included) and the smooth-rate series (and, while the Bayesian-blocks estimator is shown, its segments with change-point dates under `blocks`); a `{type: 'hello'}` message from the parent re-posts the latest stats. The parent switches views with `iframe.contentWindow.postMessage({type: 'set', params: {rate: true, est: 'blocks', p0: 0.05}}, '*')`, where `rate` is a boolean, `est` one of `smooth` | `binned` | `blocks`, and `p0` the blocks sensitivity (0.05 | 0.3 | 0.9).
