...
- Citation rate view: time-dependent citation rate (citations/year) with 68% uncertainty bands, treating citations as an inhomogeneous Poisson process. Four estimators: smooth (boundary-corrected Gaussian kernel, default), binned histogram, yearly counts (the actual number of citations per calendar year, as INSPIRE lists them), and Bayesian blocks (optimal change-point segmentation, [Scargle et al. 2013](https://arxiv.org/abs/1207.5578))
